...
export function handleMathTooltip(update: ViewUpdate) {
	...
	*/
	...
	const create = () => {
		...
		try {
			const renderedEqn = renderMath(eqnWithDecorations, ctx.mode.kind !== "inlineMath");
...
function shouldShowTooltip(state: EditorState, ctx: Context): Bounds | null {
	const eqnBounds = ctx.getMathBounds();
	if (!eqnBounds) return null;
	const settings = getLatexSuiteConfig(state);
	const isLivePreview = state.field(editorLivePreviewField);
	if (ctx.mode.kind === "blockMath" && isLivePreview && !settings.mathPreviewLivePreviewDisplay) return null;
