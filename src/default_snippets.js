[
	...
	// Environments
	{
		trigger: /(?:(pmat|bmat|Bmat|vmat|Vmat)|(matrix|cases|array))/,
		replacement: (match, options) => {
			const env = match[2] ?? `${match[1]}rix`;
			const { view, language } = options;
			const state = view.state;
			const pos = state.selection.main.head;
			const current_indentation_level = language.getIndentation(
				state,
				pos
			);
			const current_indent = language.indentString(
				state,
				current_indentation_level
			);
			const inner_indent = language.indentString(
				state,
				current_indentation_level + language.getIndentUnit(state)
			);
